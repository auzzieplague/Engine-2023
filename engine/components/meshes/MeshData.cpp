...
unsigned int MeshData::generateMeshID() {
...
void MeshData::setMaterial(const Material &material) {
    ...
    m_material = MeshData::m_material_original = material;
}
...
unsigned int MeshData::getID() const {
    return this->m_gID;
}

void MeshData::restoreMaterial() {
    this->m_material = this->m_material_original;
}

void MeshData::applyPxTransform(const physx::PxTransform &pxTransform) {
...
void MeshData::addChild(Component *child) {
    if (child->getType() == ObjectType::OT_Mesh) {
        addMesh(dynamic_cast<MeshData *>(child));
    }
    Component::addChild(child);
}

void MeshData::addMesh(MeshData *subMesh) {
