...
bool BoundingSphere::isCollidingWith(const TerrainMesh &terrain) const {
    ...
    /// subdivide the terrain mRootMesh using octree
    /// actually - all meshes can use octree to partition the faces and m_vertices


    // Check if the m_sphere is completely outside the bounding box of the mRootMesh
...
//    }
    ...
    // Check if the m_sphere intersects any of the triangles in the mRootMesh
